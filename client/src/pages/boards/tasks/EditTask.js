...
const EditTaskOverlay = ({
  TaskToEdit: { text, _id, labels, flagged, endsAt },
  setTaskToEdit,
  setEditTaskId,
  board,
  setBoard,
  ...
  useEffect(() => {
    setValues({ text, labels, flagged, endsAt });
  }, [text]);
      ...
      columns: board.columns.map((column) => {
        return {
          ...
          tasks: column.tasks.map((task) => {
            if (task._id === _id) {
              return { ...task, text: values.text, labels: values.labels, endsAt: values.endsAt };
            }
            return task;
          }),
        };
                  ...
                  <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                    <div className="flex flex-col gap-6">
                      ...
                      <div className="flex flex-col gap-10">
                        ...
                        {/* ____ endsAt ____*/}
                        <input min={new Date().toISOString().slice(0, -5)} max="2025-06-07T00:00" type="datetime-local" onChange={(e) => setValues({...values, endsAt: e.target.value})}/>
                        {/* ____ labels _____ */}
                        <div className="flex flex-col gap-2">
