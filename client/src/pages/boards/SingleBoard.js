...
import { format, formatDistance, subDays, setDefaultOptions, isSameSecond, isSameMinute, formatDistanceToNow, isSameYear, isThisSecond, isPast  } from 'date-fns'
...
const SingleBoard = ({ allProjects, setAllProjects }) => {
  ...
  useEffect(() => {
    ...
    socket.on("taskCreated", async (task) => {
      ...
      data?.payload && toast.info(`Новая задача -> ${task.text}`);
    });
    socket.on("taskDeleted", async (task) => {
      const data = await dispatch(getAllProjects());

      data?.payload && toast.info(`Задача удалена -> ${task.text}`);
    });
    // __________ Task __________
    socket.on("taskUpdated", async (task) => {
      const data = await dispatch(getAllProjects());
      data?.payload && toast.info(`Задача обновлена -> ${task.text}`);
    });

    socket.on("taskFlagged", async (task) => {
      const data = await dispatch(getAllProjects());
      data?.payload &&
        toast.info(`Задача ${task.flagged ? "под флагом!" : "без флага!"}`);
    });

    socket.on("taskDraggedInSameColumn", async (data) => {
      const result = await dispatch(getAllProjects());
      result?.payload &&
        toast.info(`Задача перемещена в ${data?.column?.name} колону`);
    });

    socket.on("taskAssigned", async (data) => {
      const result = await dispatch(getAllProjects());
      if (data?._id) {
        result?.payload && toast.info(`Задача назначена для ${data?.username} `);
      } else {
        result?.payload && toast.info(`Назначение для ${data?.username} удалено`);
      }
    });
    ...
    socket.on("taskDraggedInDifferentColumn", async (data) => {
      ...
      result?.payload &&
        toast.info(
          `Задача была перемещена от ${data?.sourceColumn?.name} к ${data?.destinationColumn?.name} колоне`
        );
    });
    ...
    socket.on("columnCreated", async (column) => {
      const result = await dispatch(getAllProjects());
      result?.payload && toast.info(`Новая колона -> ${column?.name} `);
    });

    socket.on("columnDeleted", async (column) => {
      const result = await dispatch(getAllProjects());
      result?.payload && toast.info(`${column.name} Колона удалена`);
    });

    socket.on("columnNameUpdated", async (column) => {
      const result = await dispatch(getAllProjects());
      result?.payload && toast.info(`Имя колоны была переименовано как ${column.name}`);
    });
    ...
    socket.on("columnLimitUpdated", async (column) => {
      ...
      result?.payload &&
        toast.info(
          `${column.name} Лимит задач был изменён на ${column?.limit}`
        );
    });

    socket.on("columnDragged", async (column) => {
      const result = await dispatch(getAllProjects());
      result?.payload && toast.info(`${column.name} Колона перемещена`);
    });

    // __________ Board ________
    socket.on("boardUpdated", async (board) => {
      const result = await dispatch(getAllProjects());
      result?.payload && toast.info(`"${board?.title}" Доска обновлена`);
    });

    socket.on("boardDeleted", async (board) => {
      navigate(`/project/${projectId}`);
      const result = await dispatch(getAllProjects());
      result?.payload && toast.info(`"${board?.title}" Доска удалена`);
    });
  ...
  const handleCreateTask = async (columnId) => {
    if (task.trim() === "") return toast.error("Задача не может быть пустой!");
    setBoard({
      ...board,
    ...
    setInputOpenId(null);
    setInputOpen(false);
    const data = await dispatch(CreateNewTask({ text: task, columnId, endsAt: taskEndsAt }));
  ...
  const handleDeleteColumn = async (columnId) => {
    if (board?.columns?.length === 1) {
      toast.error("Вы не можете удалить последнюю колону");
      return;
    }
  ...
  const handleChangeColumnName = async (columnId, oldColName) => {
    if (!newColName) return toast.error("Введите название новой колоны");
    else if (newColName === oldColName) {
      toast.error("Название колоны должна отличаться от других!");
      return;
    } else {
      const data = await dispatch(
  ...
  const handleAssignTask = async (member, currentTask, columnId) => {
    if (currentTask?.assignedTo && currentTask?.assignedTo?._id === member._id)
      return toast.error("Задача уже назначена этому пользователю!");
    setAssignee(member);
    setAllTasksMembers({
      ...allTasksMembers,
        ...
        <div className="flex flex-col gap-4 ">
          ...
          <div className="flex items-center gap-5">
            ...
            <button
              ...
              title={projectOwnerId !== myprofile?._id ? "Не разрешено" : "Редактировать"}
              className={`${
          ...
          <div className="others flex justify-between items-center">
            <div className="team-members text-xl">
              <div className="no-team flex gap-4 items-center text-gray-500 dark:text-slate-400">
                <span className="font-semibold">Команда из: </span>
                {allTasksMembers?.length > 0 ? (
                  ...
                  <span className="text-gray-400 dark:text-slate-400">
                    Пока что нет задач назначенных участникам
                  </span>
                )}
                <div
                  title="Чтобы добавить новых пользователей, вернитесь к проекту и нажмите на редактировать и добавить участников"
          ...
          <div className="w-full h-2 text-slate-700 text-xl dark:text-slate-300 ml-1 mt-1">
            <div className="flex h-2 gap-4 items-center">
              {(mainLoading || loading || boardLoading) && (
                <>
                  Загрузка... <AiOutlineLoading className="animate-spin-fast .4s" />
                </>
              )}
            </div>
          </div>
                        ...
                        >
                                    ...
                                    >
                                      {(provided, snapshot) => (
                                        <div
                                          ...
                                          {...provided.draggableProps}
                                          {...provided.dragHandleProps}
                                          className={`relative task cursor-default border-2 rounded-lg
                                        ...
                                        >
                                              ...
                                              {task?.labels?.map(
                                                (label, index) => {
                                                  return (
                                                    <span
                                                      key={`${label}-${index}`}
                                                      className={`bg-[#ebf1ff] text-[#0047FF] dark:bg-[#ebf1ff] inline-flex items-center ${
                                                        task?.flagged
                                                          ? "border-[.5px] border-gray-400"
                                                          : "border-[.5px] border-gray-200"
                                                      } rounded-md  px-2 py-1 text-sm capitalize font-medium `}
                                                    >
                                                      {label}
                                                    </span>
                                                  );
                                          ...
                                          {/* EndsAt */}
                                            ...
                                            {isPast(new Date(task.endsAt)) && (<div>Время закончилось</div>)}
                                            {!isPast(new Date(task.endsAt)) && (<div className="text-gray-700 dark:text-slate-200" >До окончания осталось: {formatDistanceToNow(task.endsAt, {includeSeconds: true})}</div>)}
                                              ...
                                              >
                                                {task?.assignedTo?._id ? (
                                                  <LazyLoadImage
                                                    ...
                                                    effect="blur"
                                                    title={`Назначена для ${task?.assignedTo?.email}`}
                                                    alt="Profile Picture"
                                                    className="rounded-full border-2"
                                                    width={30}
                                                    height={30}
                                                        ...
                                                        <div className="flex flex-col gap-2 h-[10rem] py-2 overflow-y-auto assignee-list-scroll">
                                                          ...
                                                          {task?.assignedTo
                                                            ...
                                                            <div
                                                              ...
                                                              title={`Назначить для ${myprofile?.email}`}
                                                            ...
                                                            (member, idx) => {
                                                              return (
                                                                <div
                                                                  ...
                                                                  title={`Назначить для ${member?.email}`}
                                                                  className="flex items-center gap-3 hover:bg-gray-100 dark:hover:bg-slate-600 p-1 rounded-md"
                                                                >
                                                                  <LazyLoadImage
                                                                    src={
                          ...
                          {inputOpen && inputOpenId == col._id && (
                            <div className="input-box p-3 rounded-lg bg-white dark:bg-slate-700">
                              ...
                              />
                                <input min={new Date().toISOString().slice(0, -5)} max="2025-06-07T00:00" className="dark:bg-slate-700 dark:text-slate-200" type="datetime-local" value={taskEndsAt} onChange={(e) => setTaskEndsAt(e.target.value)}/>
                            </div>
                          )}
