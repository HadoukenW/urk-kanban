...
const SingleProject = ({ setAllProjects }) => {
  ...
  useEffect(() => {
    ...
    socket.on("boardCreated", async (board) => {
      ...
      result?.payload && toast.info(`"${board?.title}" доска создана`);
    });
    ...
    socket.on("projectDeleted", async (project) => {
      const result = await dispatch(getAllProjects());
      result?.payload && toast.info(`"${project?.title}" проект удалён`);
      navigate("/");
    });
                ...
                <button
                  ...
                  title={
                    project?.userId?._id !== user?._id
                      ? "Только владелец проекта может редактировать проект"
          ...
          <div className="mx-16 ">
            ...
            {project?.boards?.length > 0 ? (
              ...
              <div className="flex flex-col mt-[8rem] items-center justify-center gap-5 text-2xl text-gray-400">
                <NotFound width={100} />
                <div>Начните создавать доски, чтобы увидеть их здесь.</div>
              </div>
            )}
