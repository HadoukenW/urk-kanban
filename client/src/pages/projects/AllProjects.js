...
const AllProjects = ({ allProjects, setAllProjects }) => {
  ...
  useEffect(() => {
    if (selectedFilter === "Все проекты") {
      setProjects(allProjects);
    } else if (selectedFilter === "Отмеченные звездой") {
  ...
  // ______________ Real time updates for projects ______________
  useEffect(() => {
    socket.on("projectUpdated", (project) => {
      dispatch(getAllProjects());
      dispatch(getAllUserProjects());

      toast.info(`"${project?.title}" проект обновлён`);
    });
    socket.on("projectDeleted", (project) => {
      dispatch(getAllProjects());
      toast.info(`"${project?.title}" проект удалён`);
    });
      ...
      <>
        ...
        {!loading && allProjects?.length === 0 ? (
          <div className="flex flex-col mt-[8rem] items-center justify-center gap-5 text-2xl text-gray-400">
            <NotFound width={140} />
            <div>Начните создавать проекты, чтобы они отображались здесь.</div>
          </div>
        ) : (
              ...
              <table className="table text-xl ">
                <thead>
                  <tr className="text-xl text-gray-500 dark:text-slate-300 border-b-gray-200 dark:border-b-slate-600">
                    ...
                    <th>Лидер</th>
                    <th>Команда</th>
                    <th className="flex justify-center">Больше действий</th>
                  </tr>
                </thead>
                        ...
                        >
                          <td>
                            {item?.userId?._id !== user._id ? (
                              <FaShare
                                title={`Отправлен от ${item?.userId?.username}`}
                                size={17}
                              ...
                              >
                                ...
                                >
                                  <Link
                                    ...
                                    id="menu-item-1"
                                  >
                                    <HiCursorArrowRipple size={16} /> Открыть
                                    проект
                                  </Link>
                                  <button
                                  ...
                                  <span
                                    onClick={() => {
                                      ...
                                      toast.info(
                                        `Проект скопирован в буфер обмена`
                                      );
                                      setIsOpen(false);
                                    }}
                                    ...
                                    id="menu-item-0"
                                  >
                                    <FaRegCopy size={16} /> скопировать айди проекта
                                  </span>
                                  <button
                                    onClick={() => {
        ...
        {deleteProject ? (
          <DeleteProjectOverlay
            ...
            info="Вы уверены, что хотите удалить этот проект? Все ваши данные навсегда будут удалены. Это действие необротимо."
          />
        ) : null}
      </>
    </div>
