...
export const CreateNewTask = createAsyncThunk(
  "CreateNewTask",
  async ({ text, columnId, endsAt }, { rejectWithValue }) => {
    try {
      const res = await fetch(
        `${process.env.REACT_APP_SERVER_URL}/api/task/create`,
        {
          ...
          body: JSON.stringify({ text, columnId, endsAt }),
        }
      );
...
export const updateTaskById = createAsyncThunk(
  "updateTaskById",
  async ({ taskId, text, labels, flagged, endsAt }, { rejectWithValue }) => {
    try {
      const res = await fetch(
        `${process.env.REACT_APP_SERVER_URL}/api/task/updateTask`,
        {
          ...
          body: JSON.stringify({ taskId, text, labels, flagged, endsAt }),
        }
      );
      const data = await res.json();
